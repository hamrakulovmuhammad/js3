...
let sam = cars.reduce((a, b) => {
    return a + b.engine
}, 0)
console.log(sam / 2);

let speed_car = cars.reduce((a, b) => {
...
console.log("самая быстрая машина", speed_car.engine);

let expencive = cars.reduce((a, b) => {
    if (a.price > b.price) {
        return a
    } else {
        return b
...
console.log("самая дорогая машина", expencive.price);

for (s of cars) {
    let i = s.engine * 100 / speed_car.engine
    console.log(`${s.model} ${i} %`);
}
